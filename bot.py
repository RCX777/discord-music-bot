...
@bot.command(brief='Dau din zar intre 1 si <arg>')
async def roll(ctx, max_val: int):
    # argument sanity check
    if max_val < 1:
        raise Exception('Dati si voi alta valoare')
...
# playlist - contains the name of the songs
playlist = [ "Kerosen.mp3", "What.mp3", "Camataru.mp3", "YeahYeah.mp3", "LoveSosa.mp3" ]

#play - command for playing songs from playlist
@bot.command(brief="Le spun lautarilor ce sa cante")
async def play(ctx,song):
    if not ctx.message.author.voice:
        await ctx.send("Mai intai intra intr un voice channel si dupa discutam.:kissing_heart:".format(ctx.message.author.name))
        return
    else:
        ...
        if song in playlist:
            ...
            await ctx.send('**Acum se canta: **' + song)
        else :
            await ctx.send("Nu stiu fraierii din astea")

#list - command for listing songs in playlist
@bot.command(brief="Afiseaza ce stiu lautarii sa cante")
async def list(ctx):
	for song in playlist:
		await ctx.send(song)

#disconnect - command for disconnecting the bot from the voice channel
@bot.command(brief="Iese Beleaua de la balamuc")
async def disconnect(ctx):
    voice_client = ctx.message.guild.voice_client
    if voice_client.is_connected():
        await voice_client.disconnect()
        await ctx.send("Sanatate multa v-am pupat! :muscle:")
    else:
        await ctx.send("Nu sunt cu voi pe canal")

#event for disconnecting after idling for too long
@bot.event
